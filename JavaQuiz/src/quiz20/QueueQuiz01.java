...
public class QueueQuiz01 {
	...
	public static void main(String[] args) {
		 ...
		 * arrivalTime은 업무처리 소요시간 입니다(1~3랜덤값을 초기값으로 주면됩니다.)
		 * 
		...
		for(int i =1; i<=30; i++) {
			...
			if(ran.nextInt(100)<60) { //손님이 도착했습니다!
				...
				System.out.println("customer" + i + "번째 손님이 도착했습니다.");
			}
			
			System.out.println("현재대기열:"+queue.toString());
			
			if(ran.nextInt()<50 && !queue.isEmpty()) {
				
				Customer c =queue.poll();
				sum+=c.getArrivalTime(); //시간 누적
				System.out.println(c.getName()+"의 업무가 처리되었습니다.");				
			}			
//			try {
